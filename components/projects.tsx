  ...
  return (
    <section id="projects" ref={ref} className="py-20 px-4 bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50">
      <div className="max-w-6xl mx-auto">
        <motion.h2
          className="text-4xl md:text-5xl font-bold mb-4 text-balance bg-gradient-to-r from-orange-600 to-amber-600 bg-clip-text text-transparent"
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project, index) => (
            ...
            >
              <Card className="p-6 flex flex-col hover:shadow-2xl transition-all bg-white border-2 border-orange-100 h-full">
                <h3 className="text-xl font-semibold mb-3 text-balance text-orange-900">{project.title}</h3>
                ...
                <div className="space-y-4 mt-auto">
                  <div className="grid grid-cols-2 gap-2 p-4 bg-orange-50 rounded-lg">
                    {Object.entries(project.metrics).map(([key, value]) => (
                      <div key={key} className="space-y-1">
                        ...
                        <div className="font-semibold text-orange-700">{value}</div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.tech.slice(0, 3).map((tech, i) => (
                      <Badge
                        key={i}
                        className="text-xs bg-orange-100 text-orange-700 hover:bg-orange-200 border-orange-200"
                      >
                        {tech}
                      </Badge>
                    ))}
                    {project.tech.length > 3 && (
                      <Badge className="text-xs bg-orange-100 text-orange-700 hover:bg-orange-200 border-orange-200">
                        +{project.tech.length - 3}
                      </Badge>
                    )}
                  </div>
